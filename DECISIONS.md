...
- **httpx is pinned to 0.27.2, not 0.28.** litellm 1.55 requires `httpx<0.28`. Our only httpx usage is `httpx.get` and `httpx.HTTPError`, identical across both versions, so aligning down was the smaller change. `openai` is pinned `<2` because litellm 1.55 targets the 1.x SDK surface.

## Week 7 (analyst agent)

- **The agent's tools are the project's own tested functions, not new queries.** Every tool wraps a function that already has unit tests and already serves an endpoint, so the agent cannot compute a number a different way than the dashboard does, and any figure it quotes is reproducible. This is what makes "grounded" a property of the system rather than a claim in a prompt.
- **Safety is enforced in the registry, not the prompt.** Tools are read-only (recommendations run with `persist=False`, so a question leaves no rows), arguments are coerced and clamped (limits capped at 20, windows validated by the KPI parser), and a tool failure returns `{"error": ...}` instead of raising. A prompt can be talked out of a rule; code cannot.
- **Tool budget of 6, matching the data dictionary's `tool_call_count` cap.** When the budget is spent the graph routes to a `finalize` node that asks for an answer with no tools attached, so a runaway model still returns something useful instead of looping. LangGraph's `recursion_limit` is a second guard.
- **Node functions are plain functions over a state dict; LangGraph only wires them.** The loop's behavior is unit-tested without LangGraph, without a network, and without a database (the gateway is injected). LangGraph is imported lazily, so the app boots without the `agent` extra and the endpoint returns 503 with an install hint, the same pattern as the sentiment and LLM extras.
- **Tool-calling turns are never cached.** The gateway caches ordinary completions, but each turn of an agent loop must be decided against the tool results just gathered, so `complete(tools=...)` bypasses the cache.
- **The full trace is persisted, with results truncated at 2000 characters.** Explainability (brief 11.6) needs the arguments, the outcome, and enough of the result to audit the answer; a raw timeseries would bloat the row without adding audit value.
- **LangGraph, not a hand-rolled loop.** The brief names LangGraph and it resolved cleanly against the pinned stack (tokenizers 0.21.4 and huggingface_hub 0.36.2 stay compatible with transformers 4.48). No LangChain chat-model integration is needed because the tools are our own functions and the LLM call goes through our existing gateway.
- **Failed LLM attempts store the provider's message (`llm_calls.error`).** Diagnosing a failover from `status='error'` alone meant grepping container logs. In one session two different causes appeared, a retired model and a free-tier rate limit, and they are only distinguishable from the provider text. Truncated to 500 chars so one bad reply cannot bloat the table.

## Week 8 (topics, retention, close-out)

- **BERTopic lives in an optional `topics` extra, not the app image.** It pulls umap-learn and hdbscan, which need numba, which caps numpy below 2.1, while the KPI engine pins numpy 2.2.1. Downgrading numpy to satisfy a secondary feature would put the whole analytics layer at risk, so the endpoints ship and are tested behind an injectable backend and answer 503 with an install hint when the extra is absent. Same contract the sentiment endpoints have without `nlp`.
- **Topic labels are deterministic keyword joins, not LLM-generated.** A topic whose name changes between runs makes trends impossible to follow. An LLM naming pass can be added later without changing the stored contract.
- **Topics reuse the sentiment preprocessing.** Both layers then describe the same text, so a topic's sentiment rollup is not computed over differently-cleaned input.
- **Fewer than 20 comments in a window returns `insufficient_data`.** Clustering a handful of comments produces confident-looking themes that are noise. Outliers (BERTopic's `-1`) and clusters below `min_topic_size` are never presented as topics.
- **Topic discovery is idempotent per account and window.** A re-run replaces that window's rows rather than appending, so scheduling it is safe.
- **Comments are linked to their topic via `comment_analyses.topic_id`.** Sentiment-by-topic is then a plain join instead of a second model pass.
- **Retention is a parameter with a documented default, and the purge reports what it did.** 30 days per the brief, tunable per deployment without a code change, and the job returns deleted/remaining/oldest so a scheduled run leaves evidence instead of being invisible.
